...
on the user's supported setup. Provider workflows and available features are not
identical.

Google Photos, iCloud Photos, and Amazon Photos use the same free and paid
feature limits for the shared plan entitlements. Provider-specific support claims
require current live Trash/Restore validation evidence.
