...
describe("funnel evidence", () => {
  it("derives the eight standing-board metrics deterministically", () => {
    const rows = [
      ...
      event(INSTALL_A, "app_opened", 20_000, "2026-01-08"),
    ...
    expect(
      computeFunnelMetrics(rows, { currentVersion: CURRENT_VERSION })
    ).toEqual({
      first_connect: 2,
      first_scan_started: 2,
      first_scan_completed: 1,
  ...
  it("exports only aggregate counts and distributions", () => {
    const summary = buildFunnelEvidenceSummary(
      ...
      { currentVersion: CURRENT_VERSION, from: "2026-01-01", to: "2026-01-01" }
    )
